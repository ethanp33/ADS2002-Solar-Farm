...
public class ActivityScheduleCheck<A extends Activity> {
	...
	public ActivityScheduleCheck(DateHandler date, RoomUsage usage, List<A> activities, List<? extends ActivitySchedule> schedule) {
	...
	private boolean evaluateSchedule(DateHandler date, List<A> activities, List<? extends ActivitySchedule> schedule) {
		...
		for (ActivitySchedule<?> item : schedule) {
			...
			scheduled[id] = true;
			startDay[id] = DateHandler.getDay(item.getStartTime());
			workHours[id] = date.isWorkingHours(item.getStartTime(), activities.get(id).getDuration());
		}
	...
	private double[] accumulateScheduleLoad(DateHandler date, RoomUsage usage, List<? extends ActivitySchedule> activities) {
		return isRecurring ?
			   accumulateRecurring(date, usage, activities) :
			   accumulateOnceOff(date, usage, activities);
	}

	private double[] accumulateRecurring(DateHandler date, RoomUsage usage, List<? extends ActivitySchedule> schedule) {
		...
		for (ActivitySchedule<?> scheduled : schedule) {
			...
			int period = DateHandler.getPeriod(scheduled.getStartTime());
			for (int day : date.getAllDays(scheduled.getStartTime())) {
	...
	private double[] accumulateOnceOff(DateHandler date, RoomUsage usage, List<? extends ActivitySchedule> schedule) {
